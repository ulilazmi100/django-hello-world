...
pkgs.mkShell {
  ...
  shellHook = ''
   ...
   trap \
    ...
    if ! psql -U $(whoami) -tAc "SELECT 1 FROM pg_database WHERE datname='hello_django_sql'" | grep -q 1; then
      createuser -U $(whoami)
      ...
      psql -U "$PGUSER" -d postgres -c "CREATE DATABASE hello_django_sql" || true
    fi
